# Kaun Banega CreworkPati

A game based on the popular TV show Kaun Banega Crorepati starring the legendary actor Amitabh Bachchan.

[cleckout this presentation for more infomation about the project](https://pasteapp.com/p/7Ijt5Cm9gvF/s/uFAMazF4Noc?view=a7giMKIBBOO)

## homescreen

![home screen](./kbcReadmeImages/home.png)

## Game Screen

![home screen](./kbcReadmeImages/gameScreen.png)

## Screen when the selected option is correct

![correct answer](./kbcReadmeImages/correctAns.png)

## Screen when the selected option is wrong

![wrong answer selection](./kbcReadmeImages/wrongAnsSelect.png)

## Screen when the player loses the game

![user lost](./kbcReadmeImages/wrongAnsScreen.png)

## Screen when the user wins the game

![winning screen](./kbcReadmeImages/winningScreen.png)

## Created by Team HackerBois

![HackerBois Logo](./images/HackerBois.png)

- [@Souvik](https://github.com/urlunaticguy)
- [@Shashank](https://github.com/0xShashank7)
- [@Suhail](https://github.com/suhailmshaik)
